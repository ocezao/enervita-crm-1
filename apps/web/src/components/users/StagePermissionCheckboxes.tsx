  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {[...stages].sort((a, b) => a.order - b.order).map((stage) => (
        <label key={stage.key} className="flex items-center gap-2 rounded-lg border border-border-soft bg-bg-surface-1 p-3 text-sm shadow-sm">
          <input
            type="checkbox"
          ...
          <span>
            ...
            <span className="block text-[11px] text-text-secondary">{stage.key}</span>
          </span>
        </label>
      ))}
    </div>
