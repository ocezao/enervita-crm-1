  ...
  return (
    <motion.section
      ...
      className={cn(
        ...
        dark ? 'border-slate-800/80 bg-slate-950 text-white' : 'border-white/70 bg-white/75',
...
const accentClasses: Record<string, string> = {
  orange: 'border-slate-200/80 bg-transparent text-slate-500 shadow-slate-900/5',
  green: 'border-slate-200/80 bg-transparent text-slate-500 shadow-slate-900/5',
  red: 'border-slate-200/80 bg-transparent text-slate-500 shadow-slate-900/5',
  slate: 'border-slate-200/80 bg-transparent text-slate-500 shadow-slate-900/5',
};
  ...
  return (
    <PremiumSurface className="min-h-[156px] p-4">
      <div className="relative min-h-[116px] pr-12">
        <div>
          <p className="min-h-[1.8rem] text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">{title}</p>
          <strong className="mt-2 block text-2xl font-black leading-none tracking-tight text-slate-950">{value}</strong>
          <p className="mt-2 text-[13px] font-semibold leading-5 text-slate-500">{description}</p>
        </div>
...
export function ContextHint({ text, title = 'Contexto' }: { text: string; title?: string }) {
  return (
    <div className="group relative inline-flex">
      <button type="button" aria-label={title} className="grid h-7 w-7 place-items-center rounded-full border border-white/40 bg-white/70 text-slate-500 shadow-sm backdrop-blur transition hover:text-slate-950 focus:outline-none focus:ring-4 focus:ring-orange-200">
        <HelpCircle size={15} />
      </button>
      <div className="pointer-events-none absolute right-0 top-9 z-50 w-72 translate-y-1 rounded-2xl border border-white/15 bg-slate-950/95 p-4 text-left text-xs font-semibold leading-5 text-slate-100 opacity-0 shadow-2xl shadow-slate-950/30 backdrop-blur-xl transition group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100">
