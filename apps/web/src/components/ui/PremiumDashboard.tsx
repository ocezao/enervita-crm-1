  ...
  return (
    <motion.section
      ...
      className={cn(
        ...
        dark ? 'border-graphite/80 bg-graphite text-white' : 'border-white/70 bg-white/75',
...
const accentClasses: Record<string, string> = {
  orange: 'border-warm-sand/50/80 bg-transparent text-graphite-soft shadow-graphite/5',
  green: 'border-warm-sand/50/80 bg-transparent text-graphite-soft shadow-graphite/5',
  red: 'border-warm-sand/50/80 bg-transparent text-graphite-soft shadow-graphite/5',
  slate: 'border-warm-sand/50/80 bg-transparent text-graphite-soft shadow-graphite/5',
};
  ...
  return (
    <PremiumSurface className="min-h-[156px] p-4">
      <div className="relative min-h-[116px] pr-12">
        <div>
          <p className="min-h-[1.8rem] text-[11px] font-black uppercase tracking-[0.18em] text-graphite-soft">{title}</p>
          <strong className="mt-2 block text-2xl font-black leading-none tracking-tight text-graphite">{value}</strong>
          <p className="mt-2 text-[13px] font-semibold leading-5 text-graphite-soft">{description}</p>
        </div>
  ...
  return (
    <div className="flex items-end justify-between gap-4">
      <div>
        ...
        <h2 className="mt-1 text-2xl font-black tracking-tight text-graphite">{title}</h2>
      </div>
...
export function ContextHint({ text, title = 'Contexto' }: { text: string; title?: string }) {
  return (
    <div className="group relative inline-flex">
      <button type="button" aria-label={title} className="grid h-7 w-7 place-items-center rounded-full border border-white/40 bg-white/70 text-graphite-soft shadow-sm backdrop-blur transition hover:text-graphite focus:outline-none focus:ring-4 focus:ring-orange-200">
        <HelpCircle size={15} />
      </button>
