    ...
    <div className="min-h-[calc(100vh-88px)] flex flex-col gap-4 pb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {pipelines.map((pipeline) => (
              <button
                ...
                className={cn('rounded-xl px-4 py-2 text-sm font-bold transition-colors', activePipelineKeyResolved === pipeline.key ? 'bg-solar-orange text-white shadow-sm' : 'bg-warm-sand/50 text-graphite hover:bg-warm-sand/50')}
              >
                {pipeline.label}
              </button>
            ))}
      ...
      <Card className="p-4 overflow-visible">
        <div className="grid grid-cols-1 xl:grid-cols-[1.5fr_1fr_1fr_1fr_1fr_auto] gap-3 items-center">
          <label className="relative block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-graphite-soft" size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar lead, empresa, telefone, origem ou campanha..." className="w-full bg-white border border-warm-sand/70 rounded-2xl py-3 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-solar-orange/30" /></label>
          <select value={priority} onChange={(event) => setPriority(event.target.value as 'todas' | Priority)} className="bg-white border border-warm-sand/70 rounded-2xl px-3 py-3 text-sm">{priorities.map((item) => <option key={item} value={item}>{item === 'todas' ? 'Todas prioridades' : item}</option>)}</select>
          <select value={source} onChange={(event) => setSource(event.target.value)} className="bg-white border border-warm-sand/70 rounded-2xl px-3 py-3 text-sm"><option value="todas">Todas origens</option>{sources.map((item) => <option key={item} value={item}>{item}</option>)}</select>
          <select value={aging} onChange={(event) => setAging(event.target.value as AgingFilter)} className="bg-white border border-warm-sand/70 rounded-2xl px-3 py-3 text-sm"><option value="todos">Todos tempos</option><option value="sem_proxima_acao">Sem próxima ação</option><option value="parados_3d">Parados há 3+ dias</option><option value="parados_7d">Parados há 7+ dias</option></select>
          <input value={minBill} onChange={(event) => setMinBill(event.target.value)} inputMode="numeric" placeholder="Conta mínima R$" className="bg-white border border-warm-sand/70 rounded-2xl px-3 py-3 text-sm" />
          <Button variant="outline" size="sm" className="h-11 gap-2" onClick={resetFilters}><RotateCcw size={14} /> Limpar</Button>
        </div>
        <div className="mt-3 flex flex-col md:flex-row md:items-center justify-between gap-2 text-xs text-graphite-soft"><span className="flex items-center gap-2"><Filter size={14} /> {filteredLeads.length} de {visibleRawLeads.length} leads visiveis - {urgentCount} alta/urgente</span><label className="font-bold">Ordenar <select value={sort} onChange={(event) => setSort(event.target.value as SortKey)} className="border border-warm-sand/70 rounded-xl px-2 py-1 bg-white font-medium"><option value="entry_desc">Mais recentes primeiro</option><option value="oldest_stage">Mais parados primeiro</option><option value="updated_desc">Atualizados recentemente</option><option value="bill_desc">Maior conta primeiro</option><option value="priority_desc">Prioridade comercial</option><option value="created_asc">Rotacao (distribuicao)</option></select></label></div>
      </Card>
        ...
        {visibleStages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="w-[23rem] 2xl:w-[25rem] h-full min-h-0 flex flex-col gap-3" data-testid={`pipeline-column-${stage.id}`}>
              <div className="px-1"><div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-2"><h3 className="font-black text-sm text-graphite uppercase tracking-wider">{stage.label}</h3><span className={cn('text-[10px] font-black px-1.5 py-0.5 rounded-full', overLimit ? 'bg-alert-red/10 text-alert-red' : 'bg-warm-sand/70 text-graphite')}>{columnLeads.length}</span></div><p className="text-[10px] text-graphite-soft mt-1">{stage.helper}</p></div>{canCreate && <Button variant="ghost" size="icon" className="h-7 w-7 opacity-40" aria-label={`Criar lead em ${stage.label}`} disabled title="Criação manual de lead em revisão"><Plus size={14} /></Button>}</div><div className="mt-2 flex items-center justify-between text-[10px] text-graphite-soft"><span>{formatCurrency(columnValue)}</span><span>WIP {stage.limit >= 90 ? 'livre' : stage.limit}</span></div>{overLimit && <Badge variant="error" className="mt-2">Gargalo</Badge>}{isBackwardTarget && <Badge variant="warning" className="mt-1 text-[9px]">Bloqueado para vendedores</Badge>}</div>
              <motion.div layout className={cn('flex-1 h-full min-h-0 rounded-[1.65rem] p-3 space-y-3 crm-scroll-panel overflow-y-auto overscroll-contain border-2 border-dashed transition-colors', isBackwardTarget ? 'bg-alert-red/5 border-alert-red/20 opacity-50' : draggingLeadId ? 'bg-solar-orange/5 border-solar-orange/30 shadow-inner' : 'bg-warm-sand/50/60 border-warm-sand/70/60')} onDragOver={(event) => canMoveStage && !isBackwardTarget && event.preventDefault()} onDrop={(event) => !isBackwardTarget && handleDrop(event, stage)}>
                ...
                {columnLeads.length === 0 && <div className="py-10 text-center border-2 border-dashed border-warm-sand/70 rounded-xl bg-white/60"><p className="text-[10px] font-black text-graphite-soft uppercase">{canMoveStage ? 'Solte cards aqui' : 'Sem leads nesta etapa'}</p></div>}
              </motion.div>
            </div>
          );
        })}
...
function KanbanCard({ lead, nextStage, canMoveStage, onMove, onDragStart, onDragEnd, onContextMenu }: { lead: Lead; nextStage?: PipelineColumnStage; canMoveStage: boolean; onMove: (stage: PipelineColumnStage) => void; onDragStart: () => void; onDragEnd: () => void; onContextMenu: (event: MouseEvent<HTMLDivElement>) => void }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} whileHover={{ y: -4, scale: 1.012 }} whileTap={{ scale: 0.985 }} transition={{ type: 'spring', stiffness: 430, damping: 34 }} className="bg-white rounded-2xl border border-transparent shadow-sm overflow-hidden p-4 hover:shadow-xl hover:shadow-solar-orange/10 hover:border-solar-orange/30 transition-colors group cursor-grab active:cursor-grabbing select-none" draggable={canMoveStage} onDragStart={() => { onDragStart(); }} onDragEnd={onDragEnd} onContextMenu={onContextMenu}>
      <div className="flex justify-between items-start mb-3"><div className="flex items-center gap-2"><GripVertical size={14} className="text-graphite-soft" /><PriorityBadge priority={lead.priority} /><Badge variant={entryPriority.variant}>{entryPriority.label}</Badge></div><StageBadge stage={lead.stage} /></div>
      <Link to={`/leads/${lead.id}`} className="block"><h4 className="font-black text-sm text-graphite mb-1 group-hover:text-solar-orange transition-colors">{lead.contact?.name || 'Lead sem nome'}</h4><p className="text-xs text-graphite-soft mb-2 truncate">{lead.contact?.company || lead.contact?.email || 'Sem empresa'}</p><p className="mb-3 flex items-center gap-1 text-[10px] font-bold text-graphite-soft"><CalendarClock size={12} /> Entrada {formatDate(entryDate)} - {formatElapsedSince(entryDate)}</p><div className="grid grid-cols-2 gap-2 mb-4"><div className={cn('rounded-xl p-2 text-[10px] font-bold', stalledDays >= 3 ? 'bg-alert-amber/10 text-alert-amber' : 'bg-warm-sand/50 text-graphite-soft')}><Clock size={12} className="inline mr-1" />{stalledDays === 0 ? 'Atual hoje' : `Parado ${stalledDays}d`}</div><div className={cn('rounded-xl p-2 text-[10px] font-bold', !lead.nextActionAt || (daysUntil(lead.nextActionAt) ?? 1) <= 0 ? 'bg-alert-red/10 text-alert-red' : 'bg-energy-green/10 text-energy-green')}><Phone size={12} className="inline mr-1" />{nextActionLabel(lead)}</div></div><div className="pt-3 border-t border-warm-sand/30 flex items-center justify-between gap-2"><div><p className="font-black text-sm text-energy-green">{formatCurrency(lead.energyBillValue || lead.estimatedTicket)}</p><p className="text-[10px] text-graphite-soft truncate max-w-[150px]">{lead.leadSource || 'origem indefinida'}</p></div><div className="text-right"><p className="text-[10px] font-bold text-graphite truncate max-w-[90px]">{lead.sdrOwner || 'SD'}</p><p className="text-[9px] text-graphite-soft">vendedor</p></div></div></Link>
      <div className="mt-3 grid grid-cols-2 gap-2">{whatsapp ? <a href={whatsapp} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-lg border border-warm-sand/70 px-2 py-1.5 text-xs font-bold text-graphite hover:bg-warm-sand/50">WhatsApp</a> : <span className="inline-flex items-center justify-center rounded-lg border border-warm-sand/50 px-2 py-1.5 text-xs font-bold text-graphite-soft">Sem telefone</span>}{canMoveStage && nextStage ? <Button variant="outline" size="sm" className="gap-1" onClick={() => onMove(nextStage)} aria-label={`Mover ${lead.contact?.name ?? 'lead'} para ${nextStage.label}`}><ArrowRight size={14} /> Mover</Button> : <Button variant="ghost" size="sm" disabled>Final</Button>}</div>
    </motion.div>
  );
}
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed z-50 w-[270px] overflow-hidden rounded-2xl border border-warm-sand/70 bg-white shadow-2xl shadow-graphite/20"
        style={{ left: menu.x, top: menu.y }}
        onClick={(event) => event.stopPropagation()}
        role="menu"
        aria-label={`Menu do lead ${menu.lead.contact?.name ?? 'sem nome'}`}
      >
        <div className="bg-gradient-to-r from-solar-orange/10 via-white to-energy-green/10 p-3 border-b border-warm-sand/50">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <span className="text-[11px] text-graphite-soft">{formatCurrency(menu.lead.energyBillValue || menu.lead.estimatedTicket)}</span>
            </div>
            <button type="button" onClick={onClose} className="rounded-full p-1 text-graphite-soft hover:bg-white hover:text-graphite" aria-label="Fechar menu"><X size={14} /></button>
          </div>
        </div>
        <div className="p-2">
          <Link to={`/leads/${menu.lead.id}`} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-graphite hover:bg-warm-sand/50" role="menuitem"><ExternalLink size={15} /> Abrir detalhes do lead</Link>
          {whatsapp ? <a href={whatsapp} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-graphite hover:bg-warm-sand/50" role="menuitem"><MessageCircle size={15} /> Chamar no WhatsApp</a> : <span className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-graphite-soft"><MessageCircle size={15} /> Sem telefone</span>}
          {canMoveStage && menu.nextStage ? <button type="button" onClick={() => { onMove(menu.lead, menu.nextStage!); onClose(); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-bold text-graphite hover:bg-solar-orange/10" role="menuitem"><ArrowRight size={15} /> Mover para {menu.nextStage.label}</button> : <span className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-graphite-soft"><ArrowRight size={15} /> Sem próxima etapa</span>}
