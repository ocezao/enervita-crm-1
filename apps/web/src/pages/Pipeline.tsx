          ...
          <div className="flex flex-wrap gap-2">
            {pipelines.map((pipeline) => (
              <button
                ...
                className={cn('rounded-xl px-4 py-2 text-sm font-bold transition-colors', activePipelineKeyResolved === pipeline.key ? 'bg-solar-orange text-white shadow-sm' : 'bg-warm-sand/30 text-graphite hover:bg-warm-sand/50')}
              >
                {pipeline.label}
              </button>
            ))}
        ...
        {visibleStages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="w-[23rem] 2xl:w-[25rem] h-full min-h-0 flex flex-col gap-3" data-testid={`pipeline-column-${stage.id}`}>
              <div className="px-1"><div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-2"><h3 className="font-black text-sm text-graphite uppercase tracking-wider">{stage.label}</h3><span className={cn('text-[10px] font-black px-1.5 py-0.5 rounded-full', overLimit ? 'bg-alert-red/10 text-alert-red' : 'bg-warm-sand/50 text-graphite')}>{columnLeads.length}</span></div><p className="text-[10px] text-graphite-soft mt-1">{stage.helper}</p></div>{canCreate && <Button variant="ghost" size="icon" className="h-7 w-7 opacity-40" aria-label={`Criar lead em ${stage.label}`} disabled title="Criação manual de lead em revisão"><Plus size={14} /></Button>}</div><div className="mt-2 flex items-center justify-between text-[10px] text-graphite-soft"><span>{formatCurrency(columnValue)}</span><span>WIP {stage.limit >= 90 ? 'livre' : stage.limit}</span></div>{overLimit && <Badge variant="error" className="mt-2">Gargalo</Badge>}{isBackwardTarget && <Badge variant="warning" className="mt-1 text-[9px]">Bloqueado para vendedores</Badge>}</div>
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div className="p-2">
          <Link to={`/leads/${menu.lead.id}`} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-graphite hover:bg-warm-sand/30" role="menuitem"><ExternalLink size={15} /> Abrir detalhes do lead</Link>
          {whatsapp ? <a href={whatsapp} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-graphite hover:bg-warm-sand/30" role="menuitem"><MessageCircle size={15} /> Chamar no WhatsApp</a> : <span className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-warm-sand"><MessageCircle size={15} /> Sem telefone</span>}
          {canMoveStage && menu.nextStage ? <button type="button" onClick={() => { onMove(menu.lead, menu.nextStage!); onClose(); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-bold text-graphite hover:bg-solar-orange/10" role="menuitem"><ArrowRight size={15} /> Mover para {menu.nextStage.label}</button> : <span className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-warm-sand"><ArrowRight size={15} /> Sem próxima etapa</span>}
