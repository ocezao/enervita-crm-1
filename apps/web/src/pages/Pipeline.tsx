    ...
    <div className="min-h-[calc(100vh-88px)] flex flex-col gap-4 pb-6">
      ...
      <div className="flex-1 min-h-[720px] crm-scroll-panel overflow-x-auto pb-6 -mx-2 px-2"><div className="flex gap-5 h-full min-w-max">
        {visibleStages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="w-[23rem] 2xl:w-[25rem] flex flex-col gap-3" data-testid={`pipeline-column-${stage.id}`}>
              ...
              <motion.div layout className={cn('flex-1 min-h-[640px] rounded-[1.65rem] p-3 space-y-3 crm-scroll-panel overflow-y-auto border-2 border-dashed transition-colors', draggingLeadId ? 'bg-solar-orange/5 border-solar-orange/30 shadow-inner' : 'bg-gray-100/60 border-gray-200/60')} onDragOver={(event) => canMoveStage && event.preventDefault()} onDrop={(event) => handleDrop(event, stage.id)}>
