  ...
  return (
    <div className="min-h-[calc(100vh-88px)] flex flex-col gap-4 pb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <Card className="p-4 bg-orange-500/5 border-orange-500/10"><Users size={18} className="text-orange-400" /><p className="mt-2 text-[10px] uppercase font-black text-orange-400 tracking-wider">Leads filtrados</p><strong className="text-2xl text-text-primary">{filteredLeads.length}</strong></Card>
        ...
        {visibleStages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="w-[23rem] 2xl:w-[25rem] h-full min-h-0 flex flex-col gap-3" data-testid={`pipeline-column-${stage.id}`}>
              ...
              <motion.div layout className={cn('flex-1 h-full min-h-0 rounded-[1.65rem] p-3 space-y-3 crm-scroll-panel overflow-y-auto overscroll-contain border-2 border-dashed transition-colors', isBackwardTarget ? 'bg-red-500/5 border-alert-red/20 opacity-50' : draggingLeadId ? 'bg-orange-500/5 border-orange-500/30 shadow-inner' : 'bg-bg-surface-2/50/60 border-border-strong/60')} onDragOver={(event) => canMoveStage && !isBackwardTarget && event.preventDefault()} onDrop={(event) => !isBackwardTarget && handleDrop(event, stage)}>
...
function KanbanCard({ lead, nextStage, canMoveStage, onMove, onDragStart, onDragEnd, onContextMenu }: { lead: Lead; nextStage?: PipelineColumnStage; canMoveStage: boolean; onMove: (stage: PipelineColumnStage) => void; onDragStart: () => void; onDragEnd: () => void; onContextMenu: (event: MouseEvent<HTMLDivElement>) => void }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} whileHover={{ y: -4, scale: 1.012 }} whileTap={{ scale: 0.985 }} transition={{ type: 'spring', stiffness: 430, damping: 34 }} className="bg-bg-surface-1 rounded-2xl border border-transparent shadow-sm overflow-hidden p-4 hover:shadow-xl hover:shadow-orange-500/10 hover:border-orange-500/30 transition-colors group cursor-grab active:cursor-grabbing select-none" draggable={canMoveStage} onDragStart={() => { onDragStart(); }} onDragEnd={onDragEnd} onContextMenu={onContextMenu}>
