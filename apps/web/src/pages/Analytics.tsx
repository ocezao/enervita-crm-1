...
export default function Analytics() {
  ...
  if (loading || !overview) {
    return <div className="animate-pulse text-sm text-text-secondary">Carregando analytics real do CRM...</div>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {error && (
        <Card className="p-4 border-alert-red/30 bg-red-500/5 text-alert-red text-sm flex items-center gap-2">
          <AlertTriangle size={18} /> {error}
        </Card>
      )}

      <Card className="overflow-hidden border-orange-500/20 bg-gradient-to-r from-orange-500/10 via-bg-surface-2 to-mint-500/10 p-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <h3 className="text-lg font-black text-text-primary">Painel externo de analytics</h3>
      ...
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-bold text-text-primary">Filtros de análise</h3>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.25fr_1fr_1fr_1fr] gap-4">
          ...
          <label className="space-y-1">
            <span className="text-xs font-bold uppercase tracking-wider text-text-secondary">Origem da campanha</span>
            <input list="analytics-sources" className="w-full rounded-xl border border-border-strong px-3 py-2 text-sm" placeholder="Todas" value={filters.source} onChange={(event) => setFilters((prev) => ({ ...prev, source: event.target.value }))} />
            <datalist id="analytics-sources">{sourceOptions.map((item) => <option key={item} value={item} />)}</datalist>
          </label>
          <label className="space-y-1">
            <span className="text-xs font-bold uppercase tracking-wider text-text-secondary">Campanha</span>
            <input list="analytics-campaigns" className="w-full rounded-xl border border-border-strong px-3 py-2 text-sm" placeholder="Todas" value={filters.campaign} onChange={(event) => setFilters((prev) => ({ ...prev, campaign: event.target.value }))} />
            <datalist id="analytics-campaigns">{campaignOptions.map((item) => <option key={item} value={item} />)}</datalist>
          </label>
          <label className="space-y-1">
            <span className="text-xs font-bold uppercase tracking-wider text-text-secondary">Etapa</span>
            <select className="w-full rounded-xl border border-border-strong px-3 py-2 text-sm" value={filters.stage} onChange={(event) => setFilters((prev) => ({ ...prev, stage: event.target.value as LeadStage | 'all' }))}>
          ...
          <Card key={kpi.key} className="p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="text-2xl font-display font-bold text-text-primary mt-2">{kpi.displayValue}</h3>
                <p className="text-xs text-text-secondary mt-2 leading-relaxed">{kpi.helper}</p>
              </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="p-6 xl:col-span-2">
          <h3 className="font-bold text-text-primary mb-6 flex items-center gap-2"><LineChart size={20} className="text-orange-400" /> Volume diário rastreado</h3>
        ...
        <Card className="p-6">
          <h3 className="font-bold text-text-primary mb-6 flex items-center gap-2"><Target size={20} className="text-mint-400" /> Funil CRM</h3>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="p-6 xl:col-span-2">
          <h3 className="font-bold text-text-primary mb-6 flex items-center gap-2"><TrendingUp size={20} className="text-orange-400" /> Origem, rastreio e conversão</h3>
        ...
        <Card className="p-6">
          <h3 className="font-bold text-text-primary mb-6 flex items-center gap-2"><Layers3 size={20} className="text-mint-400" /> Mix de origens</h3>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <Card className="p-6">
          <h3 className="font-bold text-text-primary mb-4 flex items-center gap-2"><ShieldCheck size={20} className="text-mint-400" /> Qualidade do rastreio</h3>
        ...
        <Card className="p-6">
          <h3 className="font-bold text-text-primary mb-4 flex items-center gap-2"><DatabaseZap size={20} className="text-orange-400" /> Eventos por plataforma</h3>
          {overview.trackingStatus.length === 0 ? (
            <div className="rounded-2xl bg-red-500/5 border border-alert-red/10 p-5 text-sm text-text-primary">Nenhum evento de campanha para os filtros atuais. A cobertura de origem dos leads já aparece ao lado; novos envios aparecerão aqui assim que forem processados.</div>
          ) : (
            <div className="space-y-3">
              {overview.trackingStatus.map((item) => (
                <div key={item.platform} className="rounded-2xl border border-border-soft p-4">
                  <div className="flex items-center justify-between"><span className="font-bold text-text-primary capitalize">{item.platform}</span><Badge variant={item.failed > 0 ? 'error' : 'success'}>{item.total} eventos</Badge></div>
                  <p className="text-xs text-text-secondary mt-2">Enviados: {item.sent} • Fila: {item.queued} • Substituídos: {item.discarded} • Falhas: {item.failed} • Último: {formatDateTime(item.lastSentAt)}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <Card className="p-6">
        <h3 className="font-bold text-text-primary mb-5 flex items-center gap-2"><Activity size={20} className="text-orange-400" /> Campanhas e leads recentes</h3>
        <div className="md:hidden divide-y divide-gray-100 rounded-2xl border border-border-soft overflow-hidden">
          {overview.campaigns.map((campaign) => (
            <article key={`${campaign.source}-${campaign.campaign}`} className="p-4 space-y-3">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-bold text-text-primary truncate">{campaign.campaign}</p><p className="text-xs text-text-secondary truncate">{campaign.source}</p></div><Badge variant="success">{campaign.trackedLeads}/{campaign.leads} rastreados</Badge></div>
              <div className="grid grid-cols-2 gap-3 text-xs text-text-secondary">
                <div><span className="block font-bold text-text-secondary uppercase">Propostas</span>{campaign.proposals}</div>
                <div><span className="block font-bold text-text-secondary uppercase">Contratos</span>{campaign.won}</div>
                <div className="col-span-2"><span className="block font-bold text-text-secondary uppercase">Pipeline</span>{formatCurrency(campaign.estimatedTicket)}</div>
              </div>
            </article>
          ))}
        </div>
