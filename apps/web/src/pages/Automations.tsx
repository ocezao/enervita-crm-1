    ...
    <div className="space-y-6 max-w-[1500px] mx-auto overflow-hidden">
        ...
        <Card className="p-6 border-solar-orange/20 bg-solar-orange/5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-graphite">Pausar interrompe novas execuções automáticas. Despausar reativa o fluxo conforme a configuração atual.</p>
      ...
      {error && !isErrorDismissed && (
        <Card className="p-4 border-alert-red/20 bg-alert-red/5 text-alert-red flex gap-3 items-start">
          ...
          <button
            ...
            className="shrink-0 rounded-full p-1 text-alert-red/70 transition hover:bg-alert-red/10 hover:text-alert-red focus:outline-none focus:ring-2 focus:ring-alert-red/30"
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-graphite-soft">Lista operacional dos fluxos publicados ou pausados para a Enervita.</p>
          </div>
          <Badge variant="info">{n8nWorkflows.length} fluxos</Badge>
        </div>

        {loading ? (
          <Card className="p-10 text-center text-graphite-soft">Carregando automações e fluxos...</Card>
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            {n8nWorkflows.map((workflow) => (
              <Card key={workflow.id} className="p-5 hover:shadow-lg transition-all border-warm-sand/50 overflow-hidden">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant={workflowBadgeVariant(workflow.status)}>{workflowStatusLabel(workflow.status)}</Badge>
                      <span className="text-[11px] font-semibold text-graphite-soft truncate">Fluxo comercial</span>
                    </div>
                    <h3 className="mt-3 text-base font-black text-graphite leading-snug break-words">{workflow.name}</h3>
                    <p className="mt-2 text-sm text-graphite leading-relaxed">{workflow.description}</p>
                  </div>
                ...
                <div className="mt-5 pt-4 border-t border-warm-sand/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <p className="text-xs text-graphite-soft">Status operacional: {workflow.active ? 'recebendo/executando' : 'sem execução automática'}</p>
                  <Button
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="text-sm text-graphite-soft">Regras comerciais registradas no CRM para alertas, tarefas e próximos passos.</p>
        </div>
            ...
            <Card key={rule.id} className="p-5 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4 gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`p-2 rounded-lg ${rule.active ? 'bg-solar-orange/10 text-solar-orange' : 'bg-warm-sand/50 text-graphite-soft'}`}><Zap size={20} /></div>
                  <div className="min-w-0">
                    <h3 className="font-bold text-graphite break-words">{rule.name}</h3>
                    <p className="text-xs text-graphite-soft break-all">Gatilho: {rule.trigger}</p>
              ...
              <div className="space-y-4 mb-6">
                <div>
                  <p className="text-[10px] font-bold text-graphite-soft uppercase tracking-widest mb-2">Condições</p>
                ...
                <div>
                  <p className="text-[10px] font-bold text-graphite-soft uppercase tracking-widest mb-2">Ações</p>
                  <div className="space-y-2">{rule.actions.map((a, i) => <div key={i} className="flex items-center gap-2 text-sm text-graphite"><ArrowRight size={14} className="text-solar-orange shrink-0" />{a}</div>)}</div>
                </div>
              </div>

              <div className="pt-4 border-t border-warm-sand/30 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-[10px] text-graphite-soft"><Clock size={12} />Executado em {rule.lastRunAt ? formatDate(rule.lastRunAt) : 'Nunca'}</div>
