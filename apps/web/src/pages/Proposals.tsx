  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-5">
          <p className="text-xs font-bold text-graphite-soft uppercase">Modelos utilizáveis</p>
          <h3 className="text-2xl font-bold text-graphite mt-1">{loading ? '...' : usableTemplates.length}</h3>
          <p className="mt-1 text-xs text-graphite-soft">Templates globais para qualquer lead.</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-bold text-graphite-soft uppercase">Leads com proposta</p>
          <h3 className="text-2xl font-bold text-solar-orange mt-1">{loading ? '...' : leadsWithProposals}</h3>
          <p className="mt-1 text-xs text-graphite-soft">Leads únicos com proposta criada.</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-bold text-graphite-soft uppercase">Economia anual em aberto</p>
          <h3 className="text-2xl font-bold text-energy-success mt-1">{formatCurrency(openValue)}</h3>
          <p className="mt-1 text-xs text-graphite-soft">Rascunhos e enviadas.</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs font-bold text-graphite-soft uppercase">Dimensionadas</p>
          <h3 className="text-2xl font-bold text-graphite mt-1">{loading ? '...' : withSolarDimensioning}</h3>
          <p className="mt-1 text-xs text-graphite-soft">{sentOrAccepted} enviadas ou aceitas.</p>
        </Card>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-[0.9fr_1.6fr] gap-6 items-start">
        <Card className="overflow-hidden">
          <div className="border-b border-warm-sand/50 bg-gradient-to-r from-solar-orange/10 via-white to-energy-green/10 p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-graphite-soft">Usáveis em todos os leads. Base para acelerar propostas recorrentes.</p>
              </div>
              ...
              <article key={template.id} className="p-5 hover:bg-warm-sand/30/70 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-graphite-soft">{template.templateName || 'Modelo sem nome interno'}</p>
                  </div>
                  <Badge variant="info">Modelo</Badge>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-graphite-soft">
                  <div><span className="block font-bold uppercase text-graphite-soft">Desconto</span>{template.discountPercentage}%</div>
                  <div><span className="block font-bold uppercase text-graphite-soft">Economia anual</span><strong className="text-energy-success"><ProposalMoney value={template.projectedAnnualSavings} /></strong></div>
                </div>
                {template.contentText && <p className="mt-3 line-clamp-2 text-sm text-graphite-soft">{template.contentText}</p>}
              </article>
            ))}
            {!loading && usableTemplates.length === 0 && (
              <div className="px-6 py-12 text-center text-graphite-soft">
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-warm-sand/50 p-5">
            <div className="flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-graphite-soft">Lista operacional para acompanhar proposta por lead e status comercial.</p>
              </div>
              <span className="text-xs font-bold text-graphite-soft">{leadProposals.length} propostas vinculadas</span>
            </div>
          </div>
          <div className="md:hidden divide-y divide-gray-100">
            {leadProposals.map((proposal) => (
              <article key={proposal.id} className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0"><p className="font-bold text-graphite truncate flex items-center gap-2"><FileText size={16} className="text-solar-orange shrink-0" />{proposal.title}</p><p className="text-xs text-graphite-soft truncate">{proposal.leadName ?? proposal.leadId}</p></div>
                  <ProposalStatusBadge proposal={proposal} />
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs text-graphite-soft">
                  <div><span className="block font-bold text-graphite-soft uppercase">Conta</span><ProposalMoney value={proposal.monthlyBillValue} /></div>
                  <div><span className="block font-bold text-graphite-soft uppercase">Desconto</span>{proposal.discountPercentage}%</div>
                  {proposal.solarSummary && <div className="col-span-2"><span className="block font-bold text-graphite-soft uppercase">Sistema</span>{proposal.solarSummary.quantidadeSugerida ?? '-'} módulos · {solarNumberText(proposal.solarSummary.potenciaTotalKwp)} kWp · {proposal.solarSummary.cidade}/{proposal.solarSummary.uf}</div>}
                  <div className="col-span-2"><span className="block font-bold text-graphite-soft uppercase">Economia anual</span><span className="font-bold text-energy-success"><ProposalMoney value={proposal.projectedAnnualSavings} /></span></div>
                </div>
              </article>
            ))}
            {!loading && leadProposals.length === 0 && <div className="px-6 py-12 text-center text-graphite-soft"><Send className="mx-auto mb-3" />Nenhum lead com proposta ainda.</div>}
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {leadProposals.map((proposal) => (
                  <tr key={proposal.id} className="hover:bg-warm-sand/30/60">
                    ...
                    <td className="px-6 py-4 text-xs text-graphite">
                    ...
                    <td className="px-6 py-4 text-graphite">{proposal.discountPercentage}%</td>
                    ...
                    <td className="px-6 py-4 text-xs text-graphite-soft">{formatDate(proposal.updatedAt || proposal.createdAt)}</td>
                  </tr>
                ))}
                {!loading && leadProposals.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-6 py-12 text-center text-graphite-soft">
