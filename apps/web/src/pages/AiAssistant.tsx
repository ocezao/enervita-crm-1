  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Consulta segura do CRM · modo read-only</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
        <section className="bg-white border border-gray-100 rounded-3xl shadow-sm min-h-[560px] flex flex-col overflow-hidden">
            ...
            {messages.map((item, index) => (
              <div key={index} className={item.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
                <div className={item.role === 'user'
                  ...
                  : 'max-w-[80%] rounded-2xl bg-gray-50 text-graphite px-4 py-3 text-sm leading-relaxed'}>
                  {item.content}
                </div>
              </div>
            ))}
          ...
          <div className="border-t border-gray-100 p-4 flex gap-3">
            <input
              ...
              className="flex-1 rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-solar-orange"
            />
            <button
              onClick={() => void send()}
              disabled={loading}
          ...
          <div className="bg-white border border-gray-100 rounded-3xl p-5 shadow-sm">
            ...
            <div className="space-y-2">
              {suggestions.map((item) => (
                <button key={item} onClick={() => void send(item)} className="w-full text-left text-sm bg-gray-50 hover:bg-mint-light/60 rounded-2xl px-3 py-3 text-gray-700">
                  {item}
                </button>
              ))}
            </div>
