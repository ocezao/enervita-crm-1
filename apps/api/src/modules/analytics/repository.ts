...
export type AnalyticsTrackingStatus = { platform: string; sent: number; queued: number; failed: number; discarded: number; total: number; lastSentAt: string | null };
  ...
  return {
    async getOverview(tenantId, allowedStages, filters = {}) {
      ...
      const [summary, daily, funnel, sources, campaigns, signals, trackingStatus, eventNames, recentLeads] = await Promise.all([
        pool.query(`with lead_base as (select l.*, (${trackedExpr}) as is_tracked from leads l where ${where.clause}) select count(*)::int as total_leads, count(*) filter (where is_tracked)::int as tracked_leads, (select count(distinct p.id)::int from proposals p join lead_base lb on lb.tenant_id = p.tenant_id and lb.id = p.lead_id where p.status in ('sent','accepted')) as proposals_sent, count(*) filter (where stage = 'contrato_enervita')::int as won_leads, coalesce(sum(estimated_ticket), 0) as estimated_pipeline, coalesce(avg(nullif(energy_bill_value, 0)), 0) as avg_bill, (select count(distinct te.id)::int from tracking_events te join lead_base lb on lb.tenant_id = te.tenant_id and lb.id = te.lead_id) as tracking_events from lead_base`, where.params),
        ...
        pool.query(`select coalesce(nullif(te.platform, ''), 'unknown') as platform, count(*) filter (where te.status = 'sent')::int as sent, count(*) filter (where te.status = 'queued')::int as queued, count(*) filter (where te.status = 'failed')::int as failed, count(*) filter (where te.status = 'discarded')::int as discarded, count(*)::int as total, max(te.sent_at)::text as last_sent_at from tracking_events te join leads l on l.tenant_id = te.tenant_id and l.id = te.lead_id where ${where.clause} group by 1 order by total desc, platform asc`, where.params),
