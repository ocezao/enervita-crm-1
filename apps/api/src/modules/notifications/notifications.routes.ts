...
export async function registerNotificationsRoutes(
  ...
  app.get("/api/notifications", { preHandler }, async (request) => {
    ...
    return options.notificationsRepository.listForUser(
      user.tenantId,
      user.id,
  ...
  app.post(
    ...
    async (request, reply: FastifyReply) => {
      ...
      const notification = await options.notificationsRepository.markAsRead(
        user.tenantId,
        user.id,
        id,
      );
  ...
  app.post("/api/notifications/read-all", { preHandler }, async (request) => {
    const user = authenticatedUser(request);
    const count = await options.notificationsRepository.markAllAsRead(
      user.tenantId,
      user.id,
    );
    return { count };
    ...
    async (request, reply: FastifyReply) => {
      ...
      if (!isAdminUser(user))
        return reply
          .code(403)
          .send({
            error: "Somente administradores podem rodar regras de notificação",
          });
      const result = await options.notificationsRepository.runCommercialRules(
        user.tenantId,
      );
      return result;
    },
